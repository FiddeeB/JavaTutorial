...
Action Save = new AbstractAction("Save", new ImageIcon("save.gif")) { //Samma som ovan
	...
	public void actionPerformed(ActionEvent e) { //Utför uppgiften
		if(!currentFile.equals("Untitled")) //Gör en if loop som säger att om filens titel är oförändrad så sparas filen 
			saveFile(currentFile);
		else //Säger att om filen inte har samma namn som sin tidigare version så andvänds saveFileAs
			saveFileAs();
	}
};				
Action Quit = new AbstractAction("Quit") { //Säger vad som ska hända när man klickar på quit
	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	...
	public void actionPerformed(ActionEvent e) {
		saveOld(); //Sparar det man gör
		System.exit(0); //Säger att den ska stänga ner allt
	}
};
ActionMap m = area.getActionMap(); //Säger var i programmet som kommande actions ska verka i
	Action Cut = m.get(DefaultEditorKit.cutAction); //Gör så att ctrl x fungerar
	Action Copy = m.get(DefaultEditorKit.copyAction);  //Gör så att ctrl c fungerar
	Action Paste = m.get(DefaultEditorKit.pasteAction); //Gör så att ctrl v fungerar

private void saveFileAs() { //Öppnar en filutforskare
	if(dialog.showSaveDialog(null)==JFileChooser.APPROVE_OPTION)
		saveFile(dialog.getSelectedFile().getAbsolutePath());
}
private void saveOld() { //Om titeln och platsen är oförändrad så sparas den bara
	if(changed) { //Om den är förändrad händer kommande
		if(JOptionPane.showConfirmDialog(this, "Would you like to save "+ currentFile +" ?","Save",JOptionPane.YES_NO_OPTION)== JOptionPane.YES_OPTION)
			saveFile(currentFile); //Öppnar en ruta som frågar om filen ska byta namn med ja och nej alternativ
	}
	
}
private void readInFile(String fileName) {
	try {
		FileReader r = new FileReader(fileName); //Skapar en filläsare
		area.read(r, null); //säger att filläsare ska vara i området area 
		r.close(); 
		...
		JOptionPane.showMessageDialog(this,"Editor can't find the file called "+fileName);	//Lägger till en exception för ett fel som har inträffat. I detta fallet finns inte filen	
	}
}
...
public static void main(String[] arg) { //Säger vad som ska hända när programmet öppnas
	new TextEditor();
}
